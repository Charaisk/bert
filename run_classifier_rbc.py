...
from fine_tuning_model import BiLSTM_CRF
        ...
        with codecs.open(input_file, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                if len(tokens) == 2:
                    ...
                    if len(contends) == 0:
                        """为了方便examples解析，因此将words和labels存入同一个列表"""
...
class NerProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        return self._create_examples(
            self._read_data(os.path.join(data_dir, FLAGS.train_file)), "train"
        )

    def get_dev_examples(self, data_dir):
        return self._create_examples(
            self._read_data(os.path.join(data_dir, FLAGS.dev_file)), "dev"
        )

    def get_test_examples(self, data_dir):
        return self._create_examples(
            self._read_data(os.path.join(data_dir, FLAGS.test_file)), "test")
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, mode):
    ...
    if len(tokens) >= max_seq_length - 1:
        ...
        # 由于bert采用token_a和token_b作为预训练数据，因此需要"[SEP]"
    ...
    for i, token in enumerate(tokens):
        ...
        segment_ids.append(label_map[labels[i]])
...
def file_based_convert_examples_to_features(
        examples, label_list, max_seq_length, tokenizer, output_file, mode='train'):
...
def create_model(bert_config, is_training, input_ids, input_mask,
                 labels, num_classes, use_one_hot_embeddings):
    ...
    used = tf.sign(tf.abs(input_ids))  # 当达到句子末尾时后面的id都是0
    sequence_lengths = tf.reduce_sum(used, reduction_indices=1)  # [batch_size] 大小的向量，包含了当前batch中的序列长度

    # hidden_unit=FLAGS.lstm_size
    bilstm_crf = BiLSTM_CRF(embedding=embedding, labels=labels, sequence_lengths=sequence_lengths,
                            max_seq_length=FLAGS.max_seq_length, num_classes=num_classes, initializers=initializers,
                            is_training=is_training)
    rst = bilstm_crf.rst_tuple()
    return rst
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(segment_ids, logits, trans):
                # 首先对结果进行维特比解码
                # crf 解码

                weight = tf.sequence_mask(FLAGS.max_seq_length)
                precision = tf_metrics.precision(segment_ids, pred_ids, num_labels,
                                                 list(range(16)), weight)
                recall = tf_metrics.recall(segment_ids, pred_ids, num_labels,
                                           list(range(16)), weight)
                f1score = tf_metrics.f1(segment_ids, pred_ids, num_labels,
                                        list(range(16)), weight)
            ...
            eval_metrics = (metric_fn, [segment_ids, logits, trans])
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(
                mode=mode,
                loss=total_loss,
...
def main(_):
    ...
    if FLAGS.do_train:
        if data_config.get('num_train_size', '') == '':
            leng = data_config['num_train_size']
        else:
            train_examples = processor.get_train_examples(FLAGS.data_dir)
            leng = len(train_examples)
        num_train_steps = int(
            leng / FLAGS.train_batch_size * FLAGS.num_train_epochs)
